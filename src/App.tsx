...
import { Home, SprayCan, Boxes, MapPinned, CalendarDays, ShieldCheck, LogOut, RefreshCw } from 'lucide-react'
...
function Dashboard() {
  ...
  async function refresh(){setLoading(true);setError('');try{setData(await loadDashboard())}catch(e:any){setError(e?.message||'ダッシュボードを読み込めませんでした。')}finally{setLoading(false)}}
  useEffect(()=>{void refresh()},[])
  const cards=[['在庫金額',data?yen.format(data.stockValue):'—'],['在庫ロット',data?`${data.stockLots}件`:'—'],['前回散布',data?.lastSpray?.date||'—'],['次回予定',data?.nextPlan?.label||'—']]
  return <div className="page"><div className="page-head"><div><p className="eyebrow">GODAI-ME YAGI ICHIBEI</p><h1>茶園防除管理</h1><p className="sub">農薬在庫・薬液調製・散布・圃場・年間計画を一元管理</p></div><div className="head-actions"><span className="status">Supabase 接続済</span><button className="icon-button" onClick={()=>void refresh()} disabled={loading}><RefreshCw size={18} className={loading?'spin':''}/></button></div></div>
    {error&&<div className="notice error dashboard-notice">{error}</div>}
    <div className="metrics">{cards.map(([label,value])=><article className="metric" key={label}><span>{label}</span><strong>{loading&&!data?'…':value}</strong></article>)}</div>
    <div className="panel-grid"><section className="panel"><div className="panel-title"><h2>前回の散布記録</h2>{data?.lastSpray?.legacyId&&<span>{data.lastSpray.legacyId}</span>}</div>{data?.lastSpray?<div className="detail-list"><div><span>散布日</span><b>{data.lastSpray.date}</b></div><div><span>調製量</span><b>{data.lastSpray.preparedL.toLocaleString()}L</b></div><div><span>目的</span><b>{data.lastSpray.target||'未入力'}</b></div><div><span>担当</span><b>{data.lastSpray.operator||'未入力'}</b></div><div><span>天候</span><b>{data.lastSpray.weather||'未入力'}</b></div><div className="detail-wide"><span>使用農薬</span><b>{data.lastSpray.chemicals.length?data.lastSpray.chemicals.join(' / '):'明細なし'}</b></div></div>:<p className="empty">散布記録はありません。</p>}</section>
      <section className="panel"><div className="panel-title"><h2>次回の散布予定</h2>{data?.nextPlan?.legacyId&&<span>{data.nextPlan.legacyId}</span>}</div>{data?.nextPlan?<div className="detail-list"><div><span>予定</span><b>{data.nextPlan.label}</b></div><div><span>対象</span><b>{data.nextPlan.target}</b></div><div><span>推奨農薬</span><b>{data.nextPlan.pesticide}</b></div><div className="detail-wide"><span>注意事項</span><b>{data.nextPlan.note||'なし'}</b></div></div>:<p className="empty">今後の予定はありません。</p>}</section></div>
  </div>
}
