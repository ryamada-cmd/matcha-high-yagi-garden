...
function AppShell({session}:{session:Session}) {
  ...
  useEffect(() => {if (!mobileMenuOpen) return;const previous = document.body.style.overflow; document.body.style.overflow = 'hidden';const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape') setMobileMenuOpen(false) };window.addEventListener('keydown', onKey);return () => { document.body.style.overflow = previous; window.removeEventListener('keydown', onKey) }}, [mobileMenuOpen])
  const menuRouteActive = ['/spray-history','/inventory','/pesticides','/plans','/fertilizers','/fertilizer-inventory','/fertilizer-plans','/settings'].some((path) => location.pathname.startsWith(path))
  return <div className="app-shell"><aside className="sidebar desktop-sidebar"><div className="brand brand-logo-block"><img className="app-brand-logo" src="/yagi-ichibei-logo.svg" alt="五代目八木一兵衛"/><span>茶園管理</span></div><nav><NavLink to="/" end><Home size={20}/>ダッシュボード</NavLink><div className="nav-section-label">防除</div><NavLink to="/sprays"><SprayCan size={20}/>散布</NavLink><NavLink to="/spray-history"><History size={20}/>散布履歴</NavLink><NavLink to="/inventory"><Boxes size={20}/>農薬在庫</NavLink><NavLink to="/pesticides"><Database size={20}/>農薬検索</NavLink><NavLink to="/plans"><CalendarDays size={20}/>年間防除計画</NavLink><div className="nav-section-label">施肥</div><NavLink to="/fertilizer-applications"><Leaf size={20}/>施肥</NavLink><NavLink to="/fertilizer-inventory"><Boxes size={20}/>肥料在庫</NavLink><NavLink to="/fertilizers"><Database size={20}/>肥料マスタ</NavLink><NavLink to="/fertilizer-plans"><CalendarDays size={20}/>年間施肥計画</NavLink><div className="nav-section-label">共通</div><NavLink to="/fields"><MapPinned size={20}/>圃場</NavLink>{role==='admin'&&<NavLink to="/settings"><Settings size={20}/>設定・監査</NavLink>}</nav><div className="sidebar-user"><span>{session.user.email}</span><button onClick={()=>void supabase.auth.signOut()}><LogOut size={17}/>ログアウト</button></div></aside>
