...
export default function ProductPackagingPage(){
  const{allowed}=useAppPermissions(),canManage=allowed('packaging.manage')
  ...
  function edit(b:ProductPackagingBatch){if(!canManage){setError('商品化実績を編集する権限がありません。');return}setBatchId(b.batchId);setProductId(b.productId);setSourceLotId(b.sourceLotId);setUnits(String(b.unitsProduced));setDate(b.date);setFacility(b.facility);setProcessingCost(String(b.processingCostYen));setOtherCost(String(b.otherCostYen));setOperator(b.operator);setNote(b.note);window.scrollTo({top:0,behavior:'smooth'})}
  async function submit(e:FormEvent){e.preventDefault();if(!canManage)return setError('商品化実績を登録・編集する権限がありません。');setBusy(true);setError('');setSuccess('');try{if(!product)throw new Error('商品を選択してください。');if(!source)throw new Error('原料ロットを選択してください。');if(count<=0)throw new Error('商品数量を入力してください。');if(inputQty===null)throw new Error('自動商品化は内容量・原料ともにg/kgの組み合わせに対応しています。');if(inputQty>available+0.0005)throw new Error(`原料在庫が不足しています。必要 ${num.format(inputQty)}${source.unit} / 利用可能 ${num.format(available)}${source.unit}`);await saveProductPackaging({batchId:batchId||undefined,productId:product.id,sourceLotId:source.id,units:count,date,facility,processingCostYen:Number(processingCost||0),otherCostYen:Number(otherCost||0),operator,note});setSuccess(batchId?'商品化実績を更新しました。':'商品化を登録し、SKU在庫へ入庫しました。');reset();await refresh()}catch(e:any){setError(e?.message||'商品化を保存できませんでした。')}finally{setBusy(false)}}
  async function remove(b:ProductPackagingBatch){if(!canManage)return setError('商品化実績を削除する権限がありません。');const r=window.prompt(`「${b.productName}」${b.unitsProduced}個の商品化を削除しますか？\n原料は在庫へ戻ります。削除理由を入力してください。`,'入力誤り');if(r===null)return;setBusy(true);setError('');setSuccess('');try{await deleteProductPackaging(b.batchId,r);setSuccess('商品化実績を削除し、原料在庫へ戻入しました。');if(batchId===b.batchId)reset();await refresh()}catch(e:any){setError(e?.message||'商品化実績を削除できませんでした。')}finally{setBusy(false)}}
  ...
  return <div className="page product-packaging-page">
    ...
    {canManage&&<form className="panel product-packaging-form" onSubmit={submit}>
