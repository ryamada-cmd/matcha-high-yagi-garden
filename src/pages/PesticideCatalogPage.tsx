...
import { AlertTriangle,BookOpen,Database,RefreshCw,Search,ShieldCheck } from 'lucide-react'
import { loadCatalogStatus,searchPesticideCatalog,syncFamic,type CatalogSearch,type CatalogStatus } from '../lib/pesticideCatalog'
...
export default function PesticideCatalogPage(){
  ...
  async function refresh(){setLoading(true);setError('');try{setStatus(await loadCatalogStatus())}catch(e:any){setError(e?.message||'農薬DBの状態を読み込めませんでした。')}finally{setLoading(false)}}
  ...
  return <div className="page pesticide-page">
    ...
    {tab==='official'&&result.official.length>0&&<div className="catalog-list">{result.official.map(r=><article className="panel catalog-card official" key={`o-${r.id}`}><div className="catalog-card-head"><div><span className="source-badge official">FAMIC公式</span><h2>{r.pesticide_name}</h2></div><code>登録 {r.registration_no||'—'}</code></div><div className="catalog-grid"><div><span>対象</span><b>{r.target_pest||r.use_purpose||'—'}</b></div><div><span>希釈/使用量</span><b>{r.dilution_or_rate||'—'}</b></div><div><span>使用時期</span><b>{r.use_timing||'—'}</b></div><div><span>本剤使用回数</span><b>{r.product_use_count||'—'}</b></div><div><span>使用方法</span><b>{r.application_method||'—'}</b></div><div><span>会社</span><b>{r.company_name||'—'}</b></div></div>{r.active_ingredient&&<p className="catalog-foot">有効成分：{r.active_ingredient}</p>}</article>)}</div>}
