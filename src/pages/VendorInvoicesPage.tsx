...
import { deleteVendorInvoice, deleteVendorInvoicePayment, loadVendorInvoiceRole, loadVendorInvoices, saveVendorInvoice, saveVendorInvoicePayment, type VendorInvoice, type VendorInvoicePayment } from '../lib/vendorInvoices'
...
export default function VendorInvoicesPage(){
  const[invoices,setInvoices]=useState<VendorInvoice[]>([]),[role,setRole]=useState(''),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState(''),[success,setSuccess]=useState('')
  ...
  async function refresh(){setLoading(true);setError('');try{const nextRole=await loadVendorInvoiceRole();setRole(nextRole);if(nextRole==='admin')setInvoices(await loadVendorInvoices());else setInvoices([])}catch(e:any){setError(e?.message||'請求書を読み込めませんでした。')}finally{setLoading(false)}}
  ...
  function editInvoice(invoice:VendorInvoice){setForm({id:invoice.id,externalInvoiceNo:invoice.externalInvoiceNo,vendor:invoice.vendor,invoiceDate:invoice.invoiceDate,paymentDueDate:invoice.paymentDueDate,scheduledPaymentDate:invoice.scheduledPaymentDate,plannedPaymentMethod:invoice.plannedPaymentMethod||'銀行振込',plannedPaymentAccount:invoice.plannedPaymentAccount,isOnHold:invoice.isOnHold,note:invoice.note,items:invoice.items.map(item=>({key:item.id,category:item.category,description:item.description,quantity:String(item.quantity),unit:item.unit,unitPrice:String(item.unitPriceYen),taxRate:String(item.taxRate),note:item.note}))});setError('');setSuccess('');window.scrollTo({top:0,behavior:'smooth'})}
  function openPayment(invoice:VendorInvoice,payment?:VendorInvoicePayment){setPaymentInvoice(invoice);setPaymentForm(blankPayment(invoice,payment));setError('');setSuccess('')}
  function closePayment(){if(busy)return;setPaymentInvoice(null);setPaymentForm(null)}

  async function submitPayment(event:FormEvent){
    event.preventDefault();if(!paymentForm||!paymentInvoice)return;setBusy(true);setError('');setSuccess('')
  ...
  if(!loading&&!isAdmin)return <div className="page vendor-invoices-page"><div className="page-head"><div><p className="eyebrow">ACCOUNTS PAYABLE</p><h1>請求書・支払管理</h1></div></div><section className="panel invoice-access-denied"><FileText size={28}/><h2>管理者専用機能です</h2><p>外部請求書と支払情報は管理者のみ閲覧・操作できます。</p></section></div>

  return <div className="page vendor-invoices-page">
    <div className="page-head"><div><p className="eyebrow">ACCOUNTS PAYABLE</p><h1>請求書・支払管理</h1><p className="sub">外部から届く請求書を明細単位で登録し、支払予定・支払実績・未払残高を管理します。</p></div><div className="head-actions"><button className="secondary-button" type="button" onClick={exportCsv} disabled={!filtered.length}><Download size={17}/>CSVエクスポート</button><button className="icon-button" type="button" onClick={()=>void refresh()} disabled={loading}><RefreshCw size={18} className={loading?'spin':''}/></button></div></div>
    ...
    <form className="panel vendor-invoice-form" onSubmit={submitInvoice}>
    ...
    <section className="panel invoice-list-section">
      <div className="panel-title"><div><h2>請求書一覧</h2><p>支払状況と期限を確認し、実際の支払いを記録します。</p></div><span className="audit-count">{filtered.length}件</span></div>
