...
export default function DailyReportsPage(){
  ...
  async function refresh(hydrateToday=false){
    ...
    try{
      ...
      if(hydrateToday){const current=r.find(x=>x.authorId===u.id&&x.reportDate===today());if(current)loadIntoForm(current)}
    }catch(e:any){setError(e?.message||'日報を読み込めませんでした。')}finally{setLoading(false)}
  }
  useEffect(()=>{void refresh(true)},[])

  function loadIntoForm(r:DailyReport){setForm({id:r.id,reportDate:r.reportDate,weatherNote:r.weatherNote,workHours:r.workHours?String(r.workHours):'',workSummary:r.workSummary,goodPoints:r.goodPoints,issues:r.issues,nextActions:r.nextActions,fieldIds:r.fields.map(f=>f.id)});setError('');setSuccess('')}
  ...
  const canEdit=(r:DailyReport)=>!!me&&(me.role==='admin'||r.authorId===me.id)
  ...
  return <div className="page daily-reports-page">
    <div className="page-head"><div><p className="eyebrow">DAILY REPORT</p><h1>日報</h1><p className="sub">その日の作業・気づき・課題・次回対応を残し、月単位で振り返れます。</p></div><div className="head-actions"><button className="secondary-button" onClick={resetForm}><Plus size={17}/>新しい日報</button><button className="icon-button" onClick={()=>void refresh(false)} disabled={loading}><RefreshCw size={18} className={loading?'spin':''}/></button></div></div>
    ...
    <form className="panel daily-report-form" onSubmit={submit}>
      <div className="panel-title"><div><h2>{form.id?'日報を編集':'日報を書く'}</h2><p>同じ担当者・同じ日付の日報は1件です。登録後も編集できます。</p></div>{form.id&&<span>編集中</span>}</div>
