...
export default function PlansPage(){
  ...
  const canManage=allowed('spray_plans.manage')
  ...
  async function refresh(){setLoading(true);setError('');try{const d=await loadAnnualPlans();setPlans(d.plans);setFields(d.fields);setPesticides(d.pesticides);if(d.plans.length && !d.plans.some(p=>p.year===year)) setYear(d.plans[0].year)}catch(e:any){setError(e?.message||'年間計画を読み込めませんでした。')}finally{setLoading(false)}}
  ...
  function beginNew(){if(!canManage)return;setEditingId(null);setForm({...emptyForm(),year});setFormOpen(true);setError('');setSuccess('');window.scrollTo({top:0,behavior:'smooth'})}
  function beginEdit(p:AnnualPlan){if(!canManage)return;setEditingId(p.id);setForm({legacyId:p.legacyId,year:p.year,month:p.month,period:p.period,fieldId:p.fieldId,allFields:p.allFields,target:p.target,pesticideId:p.pesticideId,pesticideText:p.pesticideText,fracIrac:p.fracIrac,plannedDate:p.plannedDate,executedDate:p.executedDate,status:p.status,note:p.note});setFormOpen(true);setError('');setSuccess('');window.scrollTo({top:0,behavior:'smooth'})}
  ...
  async function submit(){
    if(!canManage) return setError('年間防除計画を変更する権限がありません。')
  ...
  async function remove(p:AnnualPlan){
    if(!canManage) return setError('年間防除計画を削除する権限がありません。')
  ...
  return <div className="page master-page">
    <div className="page-head"><div><p className="eyebrow">ANNUAL PLAN</p><h1>年間防除計画</h1><p className="sub">月・旬・病害虫・推奨農薬・実施状況を年度単位で管理します。</p></div><div className="head-actions">{canManage&&<button className="secondary-button" onClick={beginNew}><Plus size={16}/>計画追加</button>}<button className="icon-button" onClick={()=>void refresh()} disabled={loading}><RefreshCw size={18} className={loading?'spin':''}/></button></div></div>
