...
import { Check, Download, Edit3, Plus, RefreshCw, RotateCcw, Search, Trash2 } from 'lucide-react'
...
export default function ExpenseClaimsPage(){
  ...
  async function approve(c:ExpenseClaim){if(!canReview)return setError('経費を承認する権限がありません。');if(!window.confirm(`${c.applicantName} / ${yen.format(c.totalAmountYen)} を承認しますか？`))return;setBusy(true);setError('');try{await reviewExpenseClaim(c.id,'APPROVE','');setSuccess(`${c.claimNo} を承認しました。`);await refresh()}catch(e:any){setError(e?.message||'承認できませんでした。')}finally{setBusy(false)}}
  ...
  return <div className="page expense-claims-page">
    <div className="page-head"><div><p className="eyebrow">EXPENSE REIMBURSEMENT</p><h1>経費精算</h1><p className="sub">複数の購入明細をまとめて申請し、承認権限を持つ担当者が承認・差戻しできます。</p></div><div className="head-actions">{canExport&&<button className="secondary-button" onClick={exportCsv}><Download size={17}/>CSVエクスポート</button>}<button className="icon-button" onClick={()=>void refresh()} disabled={loading}><RefreshCw size={18} className={loading?'spin':''}/></button></div></div>
    ...
    <section className="expense-review-section">
      ...
      <section className="panel expense-history-panel"><div className="panel-title"><div><h2>{canReview?'申請一覧・承認':'申請履歴'}</h2><p>{canReview?'申請内容を確認して承認または差戻しします。':'自分の申請状況を確認できます。差戻し分は権限があれば修正して再申請できます。'}</p></div></div>
